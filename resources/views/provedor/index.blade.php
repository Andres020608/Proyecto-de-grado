...
@section('content')
    <h1 class="text-xl font-semibold mb-6 text-gray-800">Lista de Proveedores</h1>
    ...
    <table class="w-full border border-gray-300 rounded-lg overflow-hidden text-center">
        <thead class="bg-gray-200">
            <tr>
                ...
                <th class="p-3 text-sm font-semibold text-gray-700 text-center">Documento</th>
                <th class="p-3 text-sm font-semibold text-gray-700 text-center">Dirección</th>
                <th class="p-3 text-sm font-semibold text-gray-700 text-center">Teléfono</th>
                <th class="p-3 text-sm font-semibold text-gray-700 text-center">Correo</th>
        ...
        <tbody>
            @forelse ($provedores as $provedor)
                <tr class="border-t hover:bg-gray-100 transition">
                    <td class="p-3 text-gray-800">{{ $provedor->id }}</td>
                    <td class="p-3 text-gray-800">{{ $provedor->nombre }}</td>
                    <td class="p-3 text-gray-800">{{ $provedor->documento }}</td>
                    <td class="p-3 text-gray-800">{{ $provedor->direccion ?? '-' }}</td>
                    <td class="p-3 text-gray-800">{{ $provedor->telefono ?? '-' }}</td>
                    <td class="p-3 text-gray-800">{{ $provedor->correo }}</td>
                    <td class="p-3">
                        @if ($provedor->estado === 'activo')
                    ...
                    <td class="p-3 flex justify-center gap-3">
                        {{-- Editar --}}
                        <a href="{{ route('provedor.edit', $provedor->id) }}"
                        ...
                        <form action="{{ route('provedor.destroy', $provedor->id) }}" method="POST"
                              onsubmit="return confirm('¿Eliminar este proveedor?')" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
                <tr>
                    <td colspan="8" class="p-4 text-gray-500">No hay proveedores registrados.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
